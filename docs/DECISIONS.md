...
## 2026-07-07 — Phase 6 pivoted from OpenShift Pipelines to GitHub Actions; full DevSecOps gate set built

**Decision**: The original ask for Phase 6 was "DevSecOps practices into the release pipeline as part of OpenShift Pipelines CI/CD." Mid-exploration, the user redirected to **GitHub Actions** instead ("can we have 'CI/CD maturity with github actions"), dropping OpenShift/Tekton entirely. Two follow-up choices confirmed with the user: (1) the **full DevSecOps gate set** — build+test, dependency vulnerability scanning, container image CVE scanning, CodeQL SAST, and Docker publish on merge — over a minimal build+test-only pipeline; (2) **Trivy** over OWASP Dependency-Check for dependency scanning.

**Why**: GitHub Actions is the CI platform actually reachable from this repo (no OpenShift cluster available in this environment), and the user preferred it directly once offered the choice. The full gate set was chosen over a minimal pipeline because the earlier phases already built a security-sensitive system (multi-tenant auth, PII redaction, guardrails) that benefits from every gate; Trivy was preferred over OWASP Dependency-Check because it covers both dependency *and* container image scanning with one tool/action, rather than needing separate tooling for each.

**Repo bootstrap**: The working directory was not yet a git repository at the start of this phase. Ran `git init` locally and authored all workflow files as the first real commit content (confirmed with the user via AskUserQuestion before doing so) — no GitHub remote has been added or pushed to in this session.

**What was built** — 5 workflow files under `.github/workflows/`:
- **`ci.yml`** — backend job (JDK 21, `mvn -B verify`, uploads `surefire-reports/`) + frontend job (Node 22, `npm ci`, `npm test -- --watch=false`, `npm run build`). Triggers on push/PR to main/master, with `concurrency`/`cancel-in-progress` to avoid piling up redundant runs.
- **`codeql.yml`** — CodeQL SAST, matrix `[java-kotlin, javascript]` (the `javascript` analyzer covers TypeScript automatically — confirmed current language identifiers via GitHub's own docs rather than assuming). Triggers on push/PR plus a weekly Monday cron so drift in newly-published CodeQL query packs still gets caught even without new commits.
- **`dependency-scan.yml`** — Trivy filesystem scan (`scan-type: fs`) reading `pom.xml`/`package-lock.json` directly, no build step needed first. Fails the workflow on any CRITICAL/HIGH finding (`exit-code: "1"`), uploads SARIF to the GitHub Security tab. Also runs on a weekly cron to catch newly-published CVEs against unchanged dependencies.
- **`image-scan.yml`** — matrix `[backend, frontend]`, builds each Docker image locally (`push: false, load: true` — scanning only, actual publish is gated separately in `docker-publish.yml`), then Trivy `scan-type: image` against the built image, SARIF upload with a per-component `category` so both components' results are distinguishable in the Security tab.
- **`docker-publish.yml`** — builds+pushes both images to GHCR (`ghcr.io/<owner>/documind-{backend,frontend}:{latest, <sha>}`) only after all 4 other workflows succeed for that commit.

**Trivy version pin**: `aquasecurity/trivy-action@0.36.0`. Initially drafted from memory as `@0.28.0`; corrected after verifying the actual current release directly against the `aquasecurity/trivy-action` GitHub repo rather than trusting recalled version numbers, consistent with the version-verification discipline already established for Spring AI/jjwt earlier in this project.

**`docker-publish.yml` gating-logic bug, caught before being shown as final**: the first draft used a single `workflow_run` trigger and checked only `github.event.workflow_run.conclusion` — i.e., only the *one* workflow that happened to trigger this run. That's incorrect: whichever of the 4 gating workflows finishes first would satisfy the check even though the other 3 hadn't run yet, potentially publishing an image before dependency/image/SAST scans completed. Caught this myself during review and rewrote `check-gates` to query the GitHub Actions REST API (`gh api repos/.../actions/runs?head_sha=...`) for the **latest run of every required workflow name** against the specific commit SHA, only setting `all-passed=true` once all 4 show `conclusion == "success"`. Each of the 4 gating workflows completing re-triggers this workflow via `workflow_run`, so the check naturally re-evaluates until the last one finishes.

**Frontend `Dockerfile`/`nginx.conf` gap discovered and filled**: `infra/docker-compose.yml` had referenced `frontend/Dockerfile` since Phase 1, but the file never actually existed (only `backend/Dockerfile` did) — found while checking prerequisites for `image-scan.yml`. Built a standard multi-stage image (Node 22 Alpine build stage → `nginx:1.27-alpine` runtime stage serving `dist/frontend/browser`, the exact nested output path confirmed from real `ng build` output earlier in the session) plus an `nginx.conf` with a SPA fallback (`try_files $uri $uri/ /index.html`) so client-side routes survive a hard refresh.

**Verification actually performed, not just assumed**:
- All 5 workflow YAML files validated for syntax via `npx js-yaml` (no Python available in this shell environment for the usual `python -c "import yaml"` check).
- `mvn -B verify` run locally — 134/134 tests pass, matching `ci.yml`'s exact command.
- Backend Docker image built successfully (`docker build`, 555MB), smoke-tested with dummy env vars: Spring context starts, Tomcat initializes, repository scanning finds all 9 JPA repositories, then fails cleanly on `Connection to localhost:5432 refused` (expected — no real Postgres provided) rather than any packaging/classpath error, confirming the image itself is sound.
- Frontend Docker image built and run locally; confirmed both `/` (200, serves `index.html`) and a SPA deep-link route (`/documents`, 200 via the nginx fallback rather than a raw 404) work correctly. Test containers/images cleaned up afterward.

**Not yet done / explicitly out of scope for this session**: no GitHub remote has been created or pushed to, so none of these 5 workflows have actually executed yet — they will only run once this repository is pushed to a real GitHub remote. Performance benchmarking gates (mentioned in the original OpenShift-framed ask) were dropped from scope along with the OpenShift pivot and were not rebuilt under GitHub Actions in this session.
